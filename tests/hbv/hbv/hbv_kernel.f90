...
!
      ...
      INTEGER, PARAMETER :: imodincon = 4
      INTEGER, PARAMETER :: maxout = 20
      INTEGER, PARAMETER :: maxparam = 15
      INTEGER, PARAMETER :: maxday = 15000

      CONTAINS
!
      subroutine run(itsteps,nzones,area,param,incon,prec,airt,ep,output)
      INTEGER, INTENT(IN) :: itsteps,nzones
      REAL(8), DIMENSION(nzones), INTENT(IN) :: area
      REAL(8), DIMENSION(maxparam,nzones), INTENT(IN) :: param
      REAL(8), DIMENSION(imodincon,nzones), INTENT(IN) :: incon
      REAL(8), DIMENSION(itsteps,nzones), INTENT(IN) :: prec, airt, ep
      REAL(8), DIMENSION(nzones,maxout,itsteps), INTENT(OUT) :: output 
      REAL(8), DIMENSION(itsteps) :: q, dquh
      REAL(8), DIMENSION(itsteps,nzones) :: snowd

      REAL(8) csf,ddf,tr,ts,meltt,sweq(itsteps),lprat,lp
      REAL(8) fc,beta,k0,k1,k2,lsuz,cperc,bmax,croute

      REAL(8) temp,precip,swe,rain,snow,melt
      REAL(8) etp,dmoist,moist,dq,eta,sd
      REAL(8) suz,slz,qg,q0,q1,q2

      INTEGER bql,age, i, it, irf, izone
      REAL(8) aa,ddfmin,ddfmax,ddfage, sdold
...
!	close(88)
      ...
      REAL(8), INTENT(INOUT) :: suz, slz
      REAL(8), DIMENSION(maxday), INTENT(INOUT) :: dquh
      REAL(8), INTENT(IN) :: dq, lsuz, k0, k1, k2, cperc, bmax, croute
      REAL(8), INTENT(OUT) :: q0, q1, q2, qg
      INTEGER, INTENT(OUT) :: bql

      REAL(8) :: rat, dt, bq, slzin, slzold, suzold, sum
      INTEGER :: j
...
400    continue
      ...
      REAL(8), INTENT(INOUT) :: moist
      REAL(8), INTENT(IN) :: rain,melt,etp, lp,beta,fc
      REAL(8), INTENT(OUT) :: dq,dmoist, eta
      REAL(8) :: moistold,xx
...
!     --- substract eta of soilmoisture ---
      ...
      subroutine snowmod(csf,ddf,tr,ts,melttemp,temp,precip,swe,rain,snow,melt)
       ...
       REAL(8), INTENT(IN) :: ddf,csf,tr,ts,melttemp
!       REAL(8) dsrtemp,dd1
       REAL(8), INTENT(IN) :: temp,precip
       REAL(8), INTENT(OUT) :: melt, snow, rain 
       REAL(8), INTENT(INOUT) :: swe     
       REAL(8) :: sweold
