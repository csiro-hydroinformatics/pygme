...
def compute_PmEm(rain, evap):
    ...
    if ierr > 0:
        errmsg = "c_pygme_models_hydromodels.compute_PmEm"\
                 + f" returns {ierr}"
...
def gr4j_X1_initial(Pm, Em, X1):
    ...
    if ierr > 0:
        errmsg = "c_pygme_models_hydromodels.gr4j_X1_initial"\
                 + f" returns {ierr}"
        raise ValueError(errmsg)
...
class GR4J(Model):
    ...
    def run(self):
        ...
        ierr = c_pygme_models_hydromodels.gr4j_run(uh1.nord,
                                                   ...
                                                   self.outputs)
        if ierr > 0:
            errmsg = "c_pygme_models_hydromodels.gr4j_run returns {ierr}"
            raise ValueError(errmsg)
